...
Yes, you can compare two HTML pages to check for updates or changes before triggering the email. This involves the following steps:

1. **Generate the new HTML page with the latest JIRA data.**
2. **Compare the new HTML page with the previous one.**
3. **Send the email if there are changes or if there are new incidents.**
4. **Update the stored HTML page for future comparisons.**

Here's an updated version of the shell script that incorporates these steps:
...
TEMP_HTML_FILE="/tmp/jira_issues_new.html"
PREVIOUS_HTML_FILE="/tmp/jira_issues_previous.html"
...
# Compare new HTML file with the previous one
if [ -f "$PREVIOUS_HTML_FILE" ]; then
  diff_result=$(diff $PREVIOUS_HTML_FILE $TEMP_HTML_FILE)
else
  diff_result="differences"
fi

# Send email if there are differences or if the previous HTML file doesn't exist
if [ -n "$diff_result" ]; then
  cat $TEMP_HTML_FILE | mailx -a 'Content-Type: text/html' -s "$SUBJECT" $RECIPIENT
  # Update the previous HTML file
  cp $TEMP_HTML_FILE $PREVIOUS_HTML_FILE
fi
...
1. **Fetch JIRA Issues**:
   - Uses `curl` to fetch issues from the JIRA API and stores them in `TEMP_JSON_FILE`.
   
2. **Generate HTML Content**:
   - Creates a new HTML file (`TEMP_HTML_FILE`) with the latest JIRA issues, including JavaScript for column sorting.
   
3. **Compare HTML Files**:
   - Compares the newly generated HTML file (`TEMP_HTML_FILE`) with the previously saved HTML file (`PREVIOUS_HTML_FILE`) using the `diff` command.
   
4. **Send Email if Changes Detected**:
   - If differences are detected (or if the previous HTML file doesn't exist), the script sends an email with the new HTML content.
   - Updates the previous HTML file to the new one for future comparisons.

5. **Clean Up**:
   - Removes temporary files used during the process.

### Requirements:

- Ensure `jq` is installed (`sudo apt-get install jq`).
- Ensure `mailx` is installed and configured (`sudo
